...
function openModal(stock) {
    ...
    document.getElementById('modalBigPrice').textContent = '$' + stock.price.toFixed(2);
    document.getElementById('modalBigPrice').className = 'big-price'; // reset class

    const changeText = `${formatNumber(stock.change)} (${formatNumber(stock.percent).slice(0, -1)}%)`;
    document.getElementById('modalBigChange').textContent = changeText;
    document.getElementById('modalBigChange').className = 'big-change ' + statusClass;
    ...
    const strokeColor = statusClass === 'positive' ? '#16a34a' : statusClass === 'negative' ? '#dc2626' : '#666';
    pathElement.setAttribute('stroke', strokeColor);

    const openPrice = stock.price - stock.change;
    const high = Math.max(stock.price, openPrice) + Math.random() * (stock.price / 100);
    const low = Math.min(stock.price, openPrice) - Math.random() * (stock.price / 100);
    ...
    document.getElementById('modalVolume').textContent = formatVolume(stock.price);
    document.getElementById('modalMarketCap').textContent = formatMarketCap(stock.price);
    document.getElementById('modalPE').textContent = (15 + Math.random() * 20).toFixed(2);
...
// Gán sự kiện đóng modal (lấy từ phiên bản cũ)
